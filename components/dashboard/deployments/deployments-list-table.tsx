    ...
    <div className="grid grid-cols-1 gap-4">
      {mockDeployments.map((deployment) => (
        <Link
          ...
          className="block"
        >
          <div className="bg-card border border-border rounded-lg p-6 shadow-card cursor-pointer transition-all hover:border-primary/50 hover:shadow-card hover:scale-[1.01]">
            <div className="flex items-center justify-between">
              <div className="flex-1 space-y-4">
                <div className="flex items-center justify-between mr-8">
                  <div>
                    <h2 className="text-lg font-semibold text-foreground">{deployment.name}</h2>
                    <p className="text-xs text-muted-foreground font-mono mt-1">{deployment.commitHash}</p>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-6">
                  <div>
                    ...
                    <p className="text-sm font-medium" style={{ color: '#D97706' }}>{deployment.pipeline}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Tasks</p>
                    <Badge variant="outline" className="bg-primary/10 border-primary/30 text-primary hover:bg-primary/20">
                      {deployment.tasksCount} tasks
                    </Badge>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Created</p>
                    <p className="text-sm font-medium text-foreground">{deployment.created}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">Duration</p>
                    <p className="text-sm font-medium text-foreground">{deployment.duration}</p>
                  </div>
                </div>
              </div>
